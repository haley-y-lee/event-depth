...
class UNetRecurrentPSF(nn.Module):
    ...
    def __init__(self, num_input_channels, num_output_channels=1, skip_type='sum',
                 recurrent_block_type='convlstm', activation='sigmoid', num_encoders=4, base_num_channels=32,
                 num_residual_blocks=2, norm=None, use_upsample_conv=True, psf_init='delta', scale_factor=1):
        super().__init__()
        ...
        self.psf_layer = DepthDependentPSFLayer(min_depth=2, max_depth=80, psf_init='delta', psf_size=9)
        self.scale_factor = scale_factor

    def forward(self, cur_input, prev_states, downsample=True, measure_time=True):
        """
        ...
        for i in range(N):     
            ...
            # try:
            #     print(f"[DEBUG] shape of depth_bins= {depth_bins.shape}", flush = True)
            #     print(f"[DEBUG] shape of cur_input[i]['frames']= {cur_input[i]['frames'].shape}", flush = True)
            # except:
            #     pass
