...
def main(config, resume, initial_checkpoint=None):
    ...
    validation_dataset = torch.utils.data.Subset(validation_dataset, list(range(100)))
    ...
    # check if psf_layer.psf is included in optimizer
    assert any('psf' in name for name, _ in model.named_parameters()), "❌ psf_layer.psfs is NOT in model.parameters()"
